...
int main() {
    ...
    errors = 0;
    printf("-----------Multiprocess Testing Of Shared Memory "
           "Database-----------\n");
    ...
    for(int k = 0; k < RUN_ITERATIONS; k++) {
        ...
        max_write =
            (rand()) % __TEST_MAX_POD_ENTRY__ / 2 + __TEST_MAX_POD_ENTRY__ / 2;
        ...
        if(temp_flag != write_count) {
            printf("Error! Invalid number of reads. Should be %d but only "
                   "received %d\n",
                   write_count,
                   temp_flag);
            errors++;
