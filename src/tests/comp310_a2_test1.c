...
void read_write_test(char** keys_buf,
                     char*** data_buf,
...
void write_test(char** keys_buf,
    ...
    for(int i = 0; i < __TEST_MAX_KEY__; i++) {
        generate_unique_data(data_buf[i][elem_num[i]],
                             (rand() + 5) % __TEST_MAX_DATA_LENGTH__,
                             data_buf[i],
                             k);
        temp_flag = kv_store_write(keys_buf[i], data_buf[i][elem_num[i]]);
        if(temp_flag != 0) {
...
void read_test(char** keys_buf,
    ...
    for(int i = 0; i < __TEST_MAX_KEY__; i++) {
        ...
        if(temp == NULL) {
            printf(
                "This should not return NULL. All these keys do have values\n");
            *errors += 1;
            continue;
        }
        temp_flag = -1;
...
void get_patterns(int expected_result[__TEST_MAX_POD_ENTRY__][__TEST_MAX_KEY__],
    ...
    *pattern_length = 1;
    ...
    int all_same_flag = 1;
    memset(patterns,
           __TEST_MAX_POD_ENTRY__ + 1,
           __TEST_MAX_POD_ENTRY__ * sizeof(int));
    // Find Pattern
    ...
    if(all_same_flag) {
        printf("Only returning a single value for each key. There should be "
               "multiple unless extreme collision. \n");
        *errors += __TEST_MAX_POD_ENTRY__;
    }
}

void
read_order_test(int expected_result[__TEST_MAX_POD_ENTRY__][__TEST_MAX_KEY__],
                int i,
                int* patterns,
                int pattern_length,
                int* errors) {
    ...
    for(int k = 0; k < __TEST_MAX_POD_ENTRY__; k++) {
        if(expected_result[k][i] !=
           patterns[(k + start_index) % pattern_length]) {
    ...
    for(int i = 0; i < pattern_length; i++) {
        int v = __TEST_MAX_POD_ENTRY__ - 1 - patterns[i];
        if(v >= 0 && v < pattern_length)
            FIFO_TEST[v] = 1;
    }
    for(int i = 0; i < pattern_length; i++) {
        if(FIFO_TEST[i] != 1) {
    ...
    if(read_all == NULL) {
        ...
        *errors += 1;
        ...
        if(total_read != pattern_length) {
            ...
            *errors += total_read > pattern_length ?
                           total_read - pattern_length :
                           pattern_length - total_read;
        }
        for(int k = 0;
            k < (total_read < pattern_length ? total_read : pattern_length);
            k++) {
            if(strcmp(
                   read_all[k],
                   data_buf[i][patterns[(k + start_index) % pattern_length]]) !=
               0) {
                printf("Invalid Ordering ... %d %d %d\n",
                       pattern_length,
                       patterns[pattern_length - 1],
...
int main() {
    ...
    for(int i = 0; i < __TEST_MAX_KEY__; i++) {
        ...
        for(int j = 0; j < __TEST_MAX_POD_ENTRY__; j++) {
            data_buf[i][j] =
                calloc(1, sizeof(char) * __TEST_MAX_DATA_LENGTH__ + 1);
        }
    }

    printf("-----------One Threaded Testing Of Shared Memory "
           "Database-----------\n");
    ...
    /*for(int k = 0; k < __TEST_MAX_KEY__; k++) {
        for(int l = 0; l < __TEST_MAX_POD_ENTRY__; l++) {
            printf("DATA: Bucket %d | Element %d | Key='%s' | Value='%s'\n",
                   k,
                   l,
                   keys_buf[k],
                   data_buf[k][l]);
